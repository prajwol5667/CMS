      ...
      <b-row>
        ...
        <b-col md="12">
          <b-form-group label="Upload Background">
            <croppa placeholder="Choose Image" @file-choose="addImage" @image-remove="imageRemoved" v-model="image" ref="croppa"/>
...
export default {
  ...
  methods: {
    ...
    update(){
      ...
      if(this.$v.form.$error){
        ...
        this.$http.put(this.API_ENDPOINT+'/admin/posterAd/'+this.uId, formData).then(response => {
          console.log(response.data);
          this.loading = false;
          this.$notify.success({message:'PosterAd updated successfully'});
          if(response.data.message===''){
            this.$notify.error({message:'PosterAd background image failed to remove.'});
          }
          this.closeModal();
        }).catch(err => {
          console.log(err.response)
  ...
  created(){
    this.form.type = this.uData.type;
    this.form.order = this.uData.order
    this.form.actionType = this.uData.actionType;
    this.form.status = parseInt(this.uData.status);
    if(this.uData.actionType===1){
      this.form.action = `${this.uData.gameId} | ${this.uData.action} | ${this.uData.brand} | ${this.uData.gameKindId}`
    }else{
      this.form.action = this.uData.action;
    }
    this.form.button = this.uData.button;
    this.form.css = this.uData.css;
