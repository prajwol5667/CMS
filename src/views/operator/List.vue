<template>
  <div class="animated fadeIn">
      ...
      <b-col sm="12">
        <b-card>
          <div slot="header" class="mb-0">
            ...
            <p-button type="info" class="float-right" @click.native="$router.push({path:'/operator/create'})">Add New</p-button>
          </div>
          <b-table :hover="true" responsive stacked="lg" :striped="false" :bordered="true" :small="false" :items="operator" :fields="fields">
    ...
    <b-modal ref="edit" size="lg" header-bg-variant="primary" hide-footer title="Update Operator" @hidden="clearData()">
      <UpdateOperator :uId="updateId" :uData="updateData" v-if="updateModal" @closemodal="updateList()"/>
    </b-modal>
...
export default {
  ...
  methods: {
    getOperator(page){
      this.$http.get(this.API_ENDPOINT+'/admin/member/admin?page='+page+'&size='+this.SIZE).then(response => {
        this.operator = response.data.data
        this.count = response.data.rowCount
      }).catch(err => {
        console.log(err.response)
        this.$notify.error({message: 'Failed to fetch operator list data.'})
      })
    },
    getBadge (status) {
    ...
    edit(id){
      this.$http.get(this.API_ENDPOINT+'/admin/member/admin/'+id).then(response => {
        this.$refs.edit.show();
        ...
        if(err.response.data.message==='admin.no.exist'){
          this.$notify.error({message: 'Operator does not exist.'});
        }else{
    ...
    deleteSites(){
      ...
      this.$http.delete(this.API_ENDPOINT+'/admin/member/admin/'+id).then(() => {
        this.loader = false;
        this.$refs.confirm.hide();
        
        this.getOperator(this.currentPage);

        this.$notify.success({message: 'Operator removed successfully'});
